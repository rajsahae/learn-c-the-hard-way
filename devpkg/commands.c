...
int Command_depends(apr_pool_t* p, const char* path)
{
  ...
  for (line = bgets((bNgetc)fgetc, in, '\n'); line != NULL; line = bgets((bNgetc)fgetc, in, '\n')) {
    int rc = btrimws(line);
    check(rc == BSTR_OK, "line had length 0");

    char* str = bdata(line);
    log_info("Processing depends: %s", str);
    rc = Command_install(p, str, NULL, NULL, NULL);
    check(rc == 0, "Failed to install: %s", str);
...
int Command_fetch(apr_pool_t* p, const char* url, int fetch_only)
{
  ...
  // indicates that the install needs to actually run
  return 1;
